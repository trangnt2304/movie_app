...
class MovieDetailBloc extends Bloc<MovieDetailEvent, MovieDetailState> {
  ...
  FutureOr<void> _onInitEvent(
      MovieDetailInitEvent event, Emitter<MovieDetailState> emit) {
    emit(MovieDetailReadMoreState(isReadMore: _isReadMore));
    _movieId = event.movie.id;
    loadingCastOnInit();
  }

  FutureOr<void> _onChangedReadMoreEvent(
      MovieDetailChangeReadMoreEvent event, Emitter<MovieDetailState> emit) {
  ...
  Future<void> _onLoadingCast(MovieDetailLoadingCastEvent event,
      Emitter<MovieDetailState> emitter) async {
    emit(MovieDetailLoadingCastState());
  }

  Future<void> _onDoneLoadingCast(MovieDetailDoneLoadingCastEvent event,
      Emitter<MovieDetailState> emitter) async {
    emit(MovieDetailDoneLoadingCastState(
        listCast: _listCast, isReadMore: _isReadMore));
  }
